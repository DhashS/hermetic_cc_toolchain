...
def toolchains(
    ...
    """
        ...
        with values {path, include_dirs, copts, linkopts}. When set, the zig cc
        toolchains for that os use the external sysroot (e.g. a macOS SDK). See
        the `sysroot` tag in toolchain/ext.bzl. Backward-compatible: empty by
        default.
    """

    # Repo rule attrs can't carry nested dicts, so serialize to JSON and decode
    # inside the repository / the generated BUILD.
...
def declare_files(os, sysroots_json = "{}"):
    ...
    # the sysroot's -I/-isysroot paths resolve in the exec root (see the `files`
    # attr in toolchain/ext.bzl). Keyed by the "-<os>-" triple token.
